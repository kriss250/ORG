...
<div class="page-contents">
    ...
    <form method="post" action="{{action("ProformaController@update",$proforma->idproforma)}}">
    <input type="hidden" value="{{csrf_token()}}" name="_token" />
        <input type="hidden" name="_method" value="put" />
        <div class="row">
            <div class="col-xs-5">
                <label>Institution</label>
                <input required value="{{$proforma->institution}}" class="form-control" type="text" name="company" placeholder="Company Name / Individual" />
                <label>
                    Address / Contacts
                </label>
                <textarea name="address" required class="form-control">{{$proforma->address}}</textarea>
            </div>

            <div class="col-xs-5">
                <label>Code</label>
                <div style="width:150px" class="input-group">
                    <input class="form-control" value="{{$proforma->code}}" name="code" />
                    <span class="input-group-addon">{{date("Y")}}</span>
                </div>
                <label>Due Date</label>
                <input autocomplete="off" value="{{$proforma->due_date}}" required name="due_date" type="text" style="display:block;max-width:105% !important;width:41% !important" class="form-control date-picker" />

                <label>Delivery Date</label>
                <input autocomplete="off"  name="delivery_date" type="hidden" style="display:block;max-width:none;width:100% !important" class="form-control date-picker" />

                <label>Prestation / Description</label>
                <input value="{{$proforma->description}}" autocomplete="off" required name="description" type="text" style="display:block;max-width:none;width:100% !important" class="form-control" />
                ...
                <tr class="row_{{$i}}">
                    <td>
                        ...
                        <div class="input-group">
                            <input value="{{$item->date}}" style="border:1px solid #ccc;font-size:12px;" name="date_{{$i}}" class="form-control date-input" type="text" placeholder="Y-m-d" />
                    ...
                    <td>
                        <input value="{{$item->description}}" required type="text" data-table="org_backoffice.invoice_items" data-field="description" name="desc_{{$i}}" rows="1" class="form-control suggest-input desc-input" placeholder="Description Item {{$i}}" />
                    </td>

                    <td>
                        <input required min="1" value="{{$item->days}}" name="days_{{$i}}" type="number" class="form-control days-input" />
                    </td>
                    <td>
                        <input value="{{$item->qty}}" required min="1" name="qty_{{$i}}" type="number" class="form-control qty-input" />
                    </td> 
                    <td>
                        <input value="{{$item->unit_price}}" required name="price_{{$i}}" class="form-control price-input" type="text" placeholder="Price #" />
                    </td>
                </tr>
                <?php $i++;?>
            @endforeach
