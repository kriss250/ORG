    ...
    <div class="header-print">
        ...
        <div class="print-header-desc text-right">
            <h3 class="invoice-no" style="font-weight:bold;margin-bottom:0px;color:rgb(125, 125, 125)">
                PROFORMA <br />
                {{$proforma->code}} / {{ (new Carbon\Carbon($proforma->created_at))->format("Y")}}
            </h3>
      
            <h5 style="border:1px solid rgb(100,100,100);display:inline-block;padding:10px">DATE : {{(new Carbon\Carbon($proforma->due_date))->format("d/m/Y")}}</h5>
        </div>
...
<div style="padding-left:5px" class="container-fluid">
  ...
  <div class="invoice-address col-xs-8">
    ...
    <p style="margin-top:5px;line-height:1.5">Address : {!! nl2br(htmlentities($proforma->address)) !!}</p>

    <p>&nbsp;</p>
    <p style="font-size:13px;font-weight:bold;margin-bottom:15px;">Description : {{$proforma->description}}</p>
    <!--<p>Due Date : {{ (new \Carbon\Carbon($proforma->due_date))->format("d/m/Y")}}</p>-->
  </div>
...
<?php $min = 11; $itemsno = $proforma->items->count(); ?>
...
<tfoot>
  ...
  <tr>
    ...
    <th>TOTAL</th>
    <th>
<em style="font-size:10px;">{{\App\Settings::get("currency")}}</em> {{number_format($total)}}</th>
  </tr>
</tfoot>
</table>
...
<div class="row" style="padding:15px;">
    ...
    <div style="float:right;margin-top:40px" class="col-xs-4">
        <p>Done at {{\App\Settings::get('city')}}, On {{(new Carbon\Carbon($proforma->created_at))->format("d/m/Y")}},</p>
         General Manager<br />
