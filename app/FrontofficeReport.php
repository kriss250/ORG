...
companies.name as Company,concat(adults,'/',children) as pax,
        ...
        join room_types on room_types.idroom_types = rooms.type_id
        left join companies on companies.idcompanies = reservations.company_id
        join accounts on accounts.reservation_id = idreservation where  date(checkout) > ? or ( date(checkout) > ? and date(checkout) <> ? and reservations.status not in (2,3,4) ) or (shifted=1 and date(checkout) >?)   and checked_in is not null and reservations.status not in (2,3,4) and date(checked_in) <= '$d'  order by idreservation desc",$range)];
    }
