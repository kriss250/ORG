...
class SSP {
		...
		if ( isset($request['search']) && $request['search']['value'] != '' ) {
			...
			for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
				...
				if ( $requestColumn['searchable'] == 'true' ) {
					...
					$globalSearch[] = "".$searchCol." LIKE ".$binding;
				}
			}
		}
		...
		for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
			...
			if ( $requestColumn['searchable'] == 'true' &&
			 $str != '' ) {
				...
				$columnSearch[] = "".$searchCol." LIKE ".$binding;
			}
		}
	...
	static function simple ( $request, $table, $primaryKey, $columns,$join,$connection ="mysql_pos", $where2="",$group="" )
	{
		...
		$resFilterLength = DB::connection($connection)->select(
		...
		$resTotalLength = DB::connection($connection)->select(
			"SELECT COUNT({$primaryKey}) as leng
			 FROM   `$table`"
		);
		...
		return array(
			...
			"data"            => self::data_output( $columns, $data_array )
		);
	}
	 ...
	 *  @return array          Server-side processing response array
	 */
	static function complex ( $data,$totalLength,$columns )
	{

		$recordsFiltered = count($data);

		// Total data set length
		$resTotalLength = $totalLength;
